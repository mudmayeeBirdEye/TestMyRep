 ...
 **********************************************************************************/
  ...
  if (trigger.isUpdate && trigger.isAfter){
  	List<String> employeeSizeList = Label.Territory_Employee_Size.split(';');
	Set<string> employeeSizeSet = new set<string>();
	for(String str : employeeSizeList){
		employeeSizeSet.add(str.trim());
	}
  	// Insert approval record and submit for approval automatically.
  	LeadTriggerHelperExt.insertApprvlOnLeadConvert(Trigger.new,trigger.oldMap,employeeSizeSet);  
  }
