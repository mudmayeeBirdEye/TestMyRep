trigger Implementation_After on Implementation__c (After Insert, After Update) {
  ...
  if(TriggerHandler.BY_PASS_IMPLEMENTATION_ON_AFTER){
    ...
    if(Trigger.isInsert){
      ...
      /*Fetching Accounts*/     
        Map<id,Account> acct_map = new Map<id,Account>([SELECT id, Implementation_Status__c,Implementation_Phase_Completion_Rate__c, Adoption_Phase_Completion_Rate__c, Graduation_Phase_Completion_Rate__c FROM Account Where id IN:acc_ids]);
